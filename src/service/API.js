
export const LOGIN_API =  "auth/v1/login"
export const VERIFY_AUTH_API =  "auth/v1/verify-auth"
export const UPDATE_PROFILE_API = 'user/v1/updateprofile'
export const HOME_API = 'home/v1/homedata'
export const PRODUCT_DETAIL_API = 'product/v1/product-detail'

export const SEARCH_API = 'search/v1/search-query'
export const ALL_PRODUCTS_API = 'product/v1/all-products'
export const WISHLIST_ADD_API= 'wishlist/v1/add-wishlist'
export const WISHLIST_REMOVE_API= 'wishlist/v1/remove-wishlist'
export const WISHLIST_GET_API= 'wishlist/v1/get-wishlist'
export const GET_BRAND_API= 'brand/v1/get-brands'
export const GET_BRAND_DETAIL_API= 'brand/v1/brand-detail'
export const NOTIFICATION_API = 'user/v1/notifications'
export const ADD_CART_API = 'cart/v1/add-cart'
export const GET_CART_API = 'cart/v1/get-cart'

export const CART_ACTION_API = 'cart/v1/action-cart'
export const CONTACT_FORM_API = 'user/v1/contact-us'
export const MAKE_ORDER_API = 'order/v1/make-order'
export const ADD_ADDRESS_API = 'user/v1/address'
export const GET_ADDRESS_API = 'user/v1/addresses'
export const SET_PRIMARY_ADDRESS_API = 'user/v1/set-primary-address'
export const DELETE_ADDRESS_API = 'user/v1/delete-address'
export const GET_ORDER_API = 'order/v1/get-order'
export const ORDER_DETAILS_API = 'order/v1/get-order-details'
export const GET_CATEGORY_API = 'category/v1/get-category'
export const GET_CATEGORY_DETAILS_API = 'category/v1/category-detail'